...
@distractor{code(guess = (high+low)/2), helpDescription()}
@distractor{code(high = guess), helpDescription()}
@distractor{code(low = guess), helpDescription()}
'''
...
def binary_search(data, key):
    ...
    '''@blank(A while loop is used to continuously divide the ordered data list into 2 portions and only searching for the key in the portion that could contain it.)'''
    '''@blank(The while loop continues until the key has been found, or until the value of "low" equals or surpasses the value of "high". This indicates that the entire data list has been searched.)'''
    while(not found and low <= high):
        '''@blank(Adding high and low then dividing by 2 ensures that each guess is made in the middle of the list.)'''
        guess = (high+low)//2
        ...
        if key == data[guess]:
            ...
            if (key < data[guess]):
                '''@blank("high" is "guess - 1" because in order for the code to reach this line, the key is not equal to the index of "guess" in the data list.)'''
                high = guess - 1
                ...
                '''@blank("low" is "guess + 1" because in order for the code to reach this line, the key is not equal to the index of "guess" in the data list.)'''
                low = guess + 1
...
List = [2, 4, 7, 12, 23, 25, 31, 35, 40, 41, 47]
