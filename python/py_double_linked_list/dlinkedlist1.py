...
class DoublyLinkedList:
    ...
    '''@helpDescription(The insert_at_start(data) method adds an item to the end of the linked list.)'''
    def insert_at_end(self, data):
        ...
        while n.nref is not None:
            n = n.nref
        '''@helpDescription(Because a new node is inserted at the end of the linked list, "n"'s next node is set as the new node to be inserted.)'''
        n.nref = new_node
        '''@helpDescription(The newly inserted node's "previous node" reference is set to "n", last time in the list.)'''
        new_node.pref = n

    '''@helpDescription(The delete_at_start() method deletes the first item in the linked list.)'''
    def delete_at_start(self):
        ...
        if self.start_node.nref is None:
            '''@helpDescription(The only item in the linked list is deleted by setting the start_node to None.)'''
            self.start_node = None
            return
        '''@helpDescription(If there is more than 1 item in the linked list, the start_node reference is changed to the old start start node's "next" reference (the second element in the linked list).)'''
        self.start_node = self.start_node.nref
        '''@helpDescription(The first item of the linked list is deleted by setting the new start_node's "previous node" reference is to None.)'''
