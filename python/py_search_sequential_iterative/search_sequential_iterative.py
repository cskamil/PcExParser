...
def sequential_search(data, key):
    ...
    '''@helpDescription(A while loop is used to iterate through the data until a value in "data" matches the value of "key", or until the end of the list "data" is reached.)'''
    '''@helpDescription(Index is incremented by 1 for each iteration of the while loop while the key is not found. When the key is found, the index is is that of the key in the data list.)'''
    while(not found and index < len(data)):
        '''@helpDescription(Every value in "data" is compared to "key". At any point the value in "data" matches "key", "found" is changed to "True".)'''
        if (key == data[index]):
            found = True
    ...
    '''@helpDescription("index" represents the index of the "key" in the list "data". If "key" is not found in "data", -1 is returned.)'''
    return index

# Step 2: Test the function
numbers = [10, 25, 30, 50, 55, 60, 70, 75, 80, 95]
print(sequential_search(numbers,75))