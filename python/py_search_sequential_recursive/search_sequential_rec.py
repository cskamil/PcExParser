...
def sequential_search(data, key, index=0):
    ...
    if len(data) == 0:
        ...
        index = -1
        return index
        '''@helpDescription(If the key matches the first element of the data list, the key has been found and its index is returned.)'''
    elif key == data[0]:
        return index
    # Step 1.2: Set recursion conditions
        ...
        index += 1
        '''@helpDescription(sequential_search funtion is recursively called, but the list is shortened to exclude the part we know does not match the key. The index is also passed in, because it represents the number of places in the data list that does not contain the key.)'''
