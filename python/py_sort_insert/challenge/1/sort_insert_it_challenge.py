...
@distractor{code(data[position] = data[position+1]), helpDescription()}
@distractor{code(data[position+1] = temp), helpDescription()}
@distractor{code(data[position-1] = temp), helpDescription()}
@distractor{code(data[position-1] = data[position]), helpDescription()}
'''
...
def insertionSort(data):
    ...
    for index in range(1,len(data)):
        ...
        while position > 0 and data[position-1] > temp:
            ...
            position = position-1
        '''@blank(The item to be moved is moved into the correct position)'''
        data[position] = temp
    return data
