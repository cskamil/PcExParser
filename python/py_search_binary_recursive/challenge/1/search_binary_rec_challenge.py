...
@distractor{code(low = guess - 1), helpDescription()}
@distractor{code(if (key < (low+high)//2), helpDescription()}
'''
...
def binary_search(data, key, low, high):
    ...
    '''@helpDescription(Executes if the key has not yet been found, and "data" has not been completely searched through.)'''
        ...
        if (key < data[guess]):
            high = guess
            '''@helpDescription("else" block executes if "key" is not smaller than the guessed value in the data list.)'''
        else:
            '''@blank(If the key is greater than the value guessed, "low" is set to "guess + 1" because the key can only be in the latter half of the list. +1 is added to "low" so that the "low" and "high" will equal if the list has been completely searched.)'''
            low = guess + 1
