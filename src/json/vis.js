...
function actDone_cb(rsp) {
  ...
  if (rsp.recommendation && rsp.recommendation.length > 0) {
	...
	var frameWidth = $(ui.vis.act.frame).width();
...
function actUpdGrids(doReqState, fnCb) {
  if (doReqState) {
    var seq = data.learners[getMe(true)].state.activities[data.topics[state.vis.topicIdx].id][state.vis.act.resId][state.vis.act.act.id].sequencing;
    var prg = data.learners[getMe(true)].state.activities[data.topics[state.vis.topicIdx].id][state.vis.act.resId][state.vis.act.act.id].values["p"];
    var uri = CONST.uriServer + "GetContentLevels?usr=" + state.curr.usr + "&grp=" + state.curr.grp + "&mod=user&sid=" + state.curr.sid + "&cid=" + state.curr.cid + "&lastActivityId=" + state.vis.act.act.id + "&res=-1"+"&seq="+seq+"&prg="+prg + "&updatesm=false";
...
//@@SabaDD
  logs = rsp.logs;
  ownBadges = rsp.ownBadges;
  rmcCount = rsp.rmcCount;
  updateLogs(logs);
  updateNewBadges(ownBadges);
//@@SabaDD
  var me = getMe(false);
  visAugmentData_addAvgTopic ([me]);
  visAugmentData_addAvgRes   ([me]);
...
function initUI() {
  ...
  $(document).ready(function() {
    ...
    for(j =0 ; j< p.length; j++){
      p[j].style.maxHeight = p[j].scrollHeight+ "px";
    }
    ...
    var score = $_("stuScore");
    var point = 0;
    score.innerHTML = point;
    updateAllBadges(ownBadges);
    updateLogs(logs);
    ...
    // function addScore(){
    //   point +=10;
    //   score.innerHTML = $_point; //data.learners[0].state.activities.Arithmetic_Operations.ae.ae_arithmetic_v2.values.p;
    // }
...
function loadData() {
  ...
  //$call("GET", "/um-vis-gm/data.js", null, loadData_cb, true, false);
...
function loadData_cb(res) {
  ...
  if(data.vis.userManual){
    ...
    manualLink.title = "Mastery Grids User Manual";
  }
...
function updateAllBadges(ownBadges){
  var numOfBadges = ownBadges.length;
  if( numOfBadges != 0 ){
    for( i = 0 ; i < numOfBadges; i++){
      //console.log(i);
      var placeToPut = "#"+ ownBadges[i].value;
      //console.log(placeToPut);
      var source = ownBadges[i].img_URL;
      //console.log(source);
      $(placeToPut).attr("src", source);
      $_(ownBadges[i].value).style.opacity =1;
    }
  }
  // var numOfPgB = data.badges.myBadges.ProgressBadges.length;
  // var numOfPcB = data.badges.myBadges.PrecisionBadges.length;
  // console.log(numOfPgB);
  // console.log(numOfPcB);
  // if(numOfPgB != 0){
  //   for (var i = 0 ; i< 8; i++){
  //     console.log(i);
  //     if(data.learners[0].state.badges.ProgressBadges[i].eligible == 1){
  //       var placeToPut = "#R"+ data.learners[getMe(true)].state.badges.ProgressBadges[i].value;
  //       console.log(placeToPut);
  //       var source = data.badges.myBadges.ProgressBadges[i].imgURL;
  //       console.log(source);
  //       $(placeToPut).attr("src", source);
  //       $_("R"+ data.learners[getMe(true)].state.badges.ProgressBadges[i].value).style.opacity = 1;
  //     }
  //
  //   }
  // }
  // if(numOfPcB != 0){
  //   for (var i = 0 ; i< numOfPcB; i++){
  //     console.log(i);
  //     if(data.learners[0].state.badges.PrecisionBadges[i].eligible == 1)
  //     var placeToPut = "#R"+ data.learners[0].state.badges.PrecisionBadges[i].value;
  //     var source = data.badges.myBadges.PrecisionBadges[i].imgURL;
  //     console.log(source);
  //     $(placeToPut).attr("src", source);
  //
  //   }
...
//   var totalPoint = $_("stuScore").innerHTML;
//   var source;
//   var placeToPut;
//   console.log(totalPoint);
//   if (totalPoint > 1 && totalPoint < 3 && data.learners[getMe(true)].state.badges.ProgressBadges[0].eligible == 0 ){
//     data.learners[getMe(true)].state.badges.ProgressBadges[0].eligible = 1;
//     source = data.badges.myBadges.ProgressBadges[0].imgURL;
//     placeToPut = "#R"+ data.learners[getMe(true)].state.badges.ProgressBadges[0].value;
//     $(placeToPut).attr("src", source);
//     showModal(source,data.badges.myBadges.ProgressBadges[0].congradulationsMSG);
//     $_("R"+ data.learners[getMe(true)].state.badges.ProgressBadges[0].value).style.opacity = 1;
//   }
//   else if (totalPoint>3 && totalPoint <5 && data.learners[getMe(true)].state.badges.ProgressBadges[1].eligible ==0 ) {
//     data.learners[getMe(true)].state.badges.ProgressBadges[1].eligible = 1;
//     source = data.badges.myBadges.ProgressBadges[1].imgURL;
//     placeToPut = "#R"+ data.learners[getMe(true)].state.badges.ProgressBadges[1].value;
//     $(placeToPut).attr("src", source);
//     showModal(source,data.badges.myBadges.ProgressBadges[1].congradulationsMSG );
//     $_("R"+ data.learners[getMe(true)].state.badges.ProgressBadges[1].value).style.opacity = 1;
//   }
//   else if (totalPoint>5 && totalPoint <7 && data.learners[getMe(true)].state.badges.ProgressBadges[2].eligible ==0 ) {
//     data.learners[getMe(true)].state.badges.ProgressBadges[2].eligible = 1;
//     source = data.badges.myBadges.ProgressBadges[2].imgURL;
//     placeToPut = "#R"+ data.learners[getMe(true)].state.badges.ProgressBadges[2].value;
//     $(placeToPut).attr("src", source);
//     showModal(source,data.badges.myBadges.ProgressBadges[2].congradulationsMSG);
//     $_("R"+ data.learners[getMe(true)].state.badges.ProgressBadges[2].value).style.opacity = 1;
//   }
//   else if (totalPoint>7 && totalPoint <9 && data.learners[getMe(true)].state.badges.ProgressBadges[3].eligible ==0 ) {
//     data.learners[getMe(true)].state.badges.ProgressBadges[3].eligible = 1;
//     source = data.badges.myBadges.ProgressBadges[3].imgURL;
//     placeToPut = "#R"+ data.learners[getMe(true)].state.badges.ProgressBadges[3].value;
//     $(placeToPut).attr("src", source);
//     showModal(source,data.badges.myBadges.ProgressBadges[3].congradulationsMSG);
//     $_("R"+ data.learners[getMe(true)].state.badges.ProgressBadges[3].value).style.opacity = 1;
//   }
//
// }

function updateNewBadges(ownBadges){
  updateAllBadges(ownBadges);
  for(i = 0 ; i < ownBadges.length; i++){
    var msg = [ownBadges[i].img_URL , ownBadges[i].congradulationMSG , ownBadges[i].value, i*200];
    //console.log("msg" + msg);
    if(msg != null){
      showModal(msg[0], msg[1], msg[2],msg[3]);
    }
  }
}
function updateLogs(logs){
  if(logs.length > 0 ){
    $_("stuScore").innerHTML= logs[0].totalPoint;

    $_("firstLog").innerHTML = logs[0].description ;
  }
  if(logs.length > 1 ){
    $_("secondLog").innerHTML= logs[1].description ;
  }
  if(logs.length > 2){
    $_("thirdLog").innerHTML = logs[2].description ;
  }
  if(logs.length > 3){
    $_("forthLog").innerHTML = logs[3].description ;
  }
  if(logs.length > 4){
  $_("fifthLog").innerHTML = logs[4].description ;
  }

}

function showModal(img, text , value, delay){
  ...
  if( type == "R"){
    id = "myModalRec";
    srcFile = "ModalImageRec";
    textMsg = "congrateRec";
  }
  else if ( type == "E") {
    id = "myModalEx";
    srcFile = "ModalImageEx";
    textMsg = "congrateEx";
  }
  else if (type == "C") {
    if(value.substr(0,2) == "CH"){
      id = "myModalCh";
      srcFile = "ModalImageCh";
      textMsg = "congrateCh";
    }
    else {
      id = "myModalEx";
  ...
  var modal = $_(id);
  var span = $_Class("close")[0];
  modal.style.display = "block";
  $("#" + srcFile).attr("src",img);
  $_(textMsg).innerHTML = text;
  ...
  //console.log("img: "+ img + " text: "+ text+ " value: "+ value);
}

function makeBig(x){
  $(".popupImage").attr("src",x.childNodes[0].getAttribute("src"));
}
