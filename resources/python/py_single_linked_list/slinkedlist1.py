'''
@goalDescription(Create the necessary classes for a Singly Linked List, containing the method append(item) to add a new item to the end of the linked list. Test the singly linked list after the classes are created.)
...
class SLinkedList:
    ...
    '''@helpDescription(The append method adds an item to the end of the linked list. We must traverse the list to the end to add the item.)'''
    def append(self, item):
        '''@helpDescription(A new node called "temp" is created for the item to be added.)'''
        temp = SLinkedListNode(item, None)
        '''@helpDescription(If self.head is None, it means the linked list is currently empty, and the new node becomes the head.)'''
        if (self.head == None):
            self.head = temp
        else:
            '''@helpDescription(If the linked list already has nodes in it, "current" is set to self.head and the linked list is traversed through by a while loop until we reach the end.)'''
            current = self.head
            '''@helpDescription(We know we have reached the end of the linked list when the next node is None.)'''
            while (current.getNext() != None):
                current = current.getNext()
            '''@helpDescription(When we reach the end of the linked list, the new item is appended to the end by setting the former last item item's "next" to the new item.)'''
