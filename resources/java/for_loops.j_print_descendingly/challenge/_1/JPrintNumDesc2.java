...
/**
@goalDescription(Construct a program to accept a positive integer (N) and then print all the positive numbers in the sequence N, N-2, N-4, N-6, N-8, ... .)
@input(10)
@name(Printing Numbers in Descending Order in Decrements of 2)
@distractor{code(for (int num = 0; num < N; num += 2) {), helpDescription()}
@distractor{code(for (int num = N; num > 0; num -= 1) {), helpDescription()}
@distractor{code(for (int num = N; num >= 0; num -= 2) {), helpDescription()}
@distractor{code(for (int num = 1; num <= N; num -= 2) {), helpDescription()}
 **/
import java.util.Scanner;

public class JPrintNumDesc2 {
	...
	public static void main(String[] args) {	
		...
		/**@blank(We need to repeat printing the numbers that are in the sequence. To do this, we need to use a for loop. In the for loop, we initialize the loop control variable (called num) to N, loop until num has not reached 0, and decrement num by 2 after each iteration of the loop.)**/
		...
		/**@blank(Here, we want the for loop to start counting from the user integer (stored in variable N), go backward decrementing each number by 2 down to but not including 0 (0 is not positive). In the first part of the loop header, we define a control variable num and initialize it to N. In the second part of the loop header, we test whether the control variable has reached the termination threshold. The loop is repeated if the control variable is greater than 0 and terminates when the control variable reaches 0. In the third part of the loop header, we decrement the control variable by 2 after each iteration of the loop (because the consecutive numbers in the sequence differ by 2). )**/
		/**@blank(Note that the first part is executed once before the loop starts, the second part is executed before each iteration, and the third part is executed after each iteration of the loop. Also, note that the decrement operator (-=) subtract 2 from the value of variable num and the result is stored back into the variable num. Therefore, it is functionally equivalent to the statement num = num - 2.)**/
